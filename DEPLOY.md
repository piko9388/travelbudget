# 사내 Flask 서버 업로드 방법 (v9.8)

## 0. 준비물

- 패키지 `travelbudget_flask_v9.8.zip` (파일명 전부 영문 — 사내 압축 해제 문제 없음)
...
좌측 하단에 **v9.8 · 2026-07-27** 이 보이면 이 버전이 올라간 것입니다.
...
v9.8 변경은 **화면(HTML·JS)과 계산 표시**만이라 `data.json` 을 손대지 않습니다.
