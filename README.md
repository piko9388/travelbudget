...
2. `webmain.py`에 2줄 추가
   ...
   ```
   → `http://material.skhynix.com/travelbudget` 접속. (data_json/은 첫 실행 시 자동 생성·시드)
...
> 실제로 v9.4에 데이터를 쌓아 v10.28로 올리고 다시 되돌리는 리허설을 돌려 확인한 절차입니다.
...
| `routes.py` | API 26개 + 화면 3개 (예산 담당자 행위는 X-Admin-PW 서버 검증, `core.locked` 경계) |
...
- 팀명·코드 중복, 메일 형식·중복, 4자 미만 비밀번호는 서버가 거부합니다
- 저장은 감사 로그에 남습니다. 읽기·쓰기 **둘 다 서버에서 인증을 요구**합니다
...
- 직접 링크: `http://material.skhynix.com/travelbudget/guide`
...
| 명령 | 필요 조건 | 결과 | 운영 데이터 |
|---|---|---|---|
| `python3 test_api.py` | Flask만 | **569 passed / 0 failed** | 임시 폴더에서만 동작 (건드리지 않음) |
...
| `node tools/e2e/e2e.mjs` | Node 18+ · Playwright | **233 passed / 0 failed** | 임시 폴더 |
