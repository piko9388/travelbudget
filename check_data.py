...
def show(tag, info):
    print(f"\n[{tag}]  {info['dir']}")
    if "error" in info:
        print(f"  ⚠ data.json 을 읽을 수 없습니다 — {info['error']}")
...
""")
    ...
    if len(found) > 1:
        print("  ⚠ data.json 이 두 곳에 있습니다. 앱이 쓰는 것은 위쪽 하나뿐입니다.")
    ...
    if inside:
        print("  ⚠ 데이터가 앱 폴더 **안**에 있습니다 — 폴더를 통째로 지우고 덮으면 날아갑니다.")
        print(BAR)
        print(f"""
데이터 위치: {info['dir']}
...
■ 안전한 방법 A — 파일 5개만 교체 (권장, 가장 안전)
    v10.18 에서 바뀐 것은 이 5개뿐입니다. 나머지는 손대지 않습니다.
...
■ 절차
    1. 원장 백업        cp -r {info['dir']} ~/tb_backup_$(date +%Y%m%d)
    2. servera/travelbudget/ 폴더를 v10.18 것으로 교체
    3. 재기동 → 좌측 하단 v10.18 · 2026-08-02 확인
""")
...
■ 되돌리기
    ...
    v10.18 는 출장·예산 데이터에 새 필드를 쓰지 않으므로, 되돌려도 이전 버전이 그대로 읽습니다.
