...
■ 안전한 방법 A — 파일 5개만 교체 (권장, 가장 안전)
    v10.14 에서 바뀐 것은 이 5개뿐입니다. 나머지는 손대지 않습니다.
...
■ 절차
    1. 원장 백업        cp -r {info['dir']} ~/tb_backup_$(date +%Y%m%d)
    2. servera/travelbudget/ 폴더를 v10.14 것으로 교체
    3. 재기동 → 좌측 하단 v10.14 · 2026-08-01 확인
""")
...
■ 되돌리기
    ...
    v10.14 는 출장·예산 데이터에 새 필드를 쓰지 않으므로, 되돌려도 이전 버전이 그대로 읽습니다.
